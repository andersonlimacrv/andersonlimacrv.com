...
- [x] 3.2 `BlueprintMorphStart.astro`: cantoneiras 2px nos 4 cantos do box do `img` (contraste ≥3:1), ghost circle Ø160 + crosshair + leader, e **raio anotado** (`r 80px`) com linha de cota
- [x] 3.3 `BlueprintMorphBoard.astro` (novo): camada de fundo atrás da foto (`z-index < figure`) — quadriculado **na escala da figura (célula fixa ~20px)** em toda a área do frame + **legenda centralizada** com fundo sólido, nome real da figura, glifos proporcionais, dados agrupados (INICIAL/FINAL/POSIÇÃO) e bordas `var(--radius)`, sem sombra
...
- [x] 3.8 Responsividade: <768px tipografia reduzida, grupos empilhados, leader oculto; sem overflow
...
- [x] 4.2 `npm run test:e2e` → 52 testes (8 novos para board/raio/cantoneiras-cruz/opacidade)
