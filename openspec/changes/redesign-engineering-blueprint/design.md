...
- **Legenda com identidade visual do projeto** (mono/uppercase/tracking, bordas `var(--radius)`, sem sombra), figura identificada (`IMG.-01`) e dados agrupados (INICIAL / FINAL / POSIÇÃO).
- **Raio atual anotado** sobre a imagem (`r 80px`) no quadrante inferior esquerdo do círculo-fantasma.
...
Sem `box-shadow`; `border: 1px solid var(--border)` + `border-radius: var(--radius)` (variável do projeto, como Eyebrow/chips — bordas "conforme o padrão de todo o projeto"); cabeçalho com chip da figura (`IMG.-01`, estilo Eyebrow) + régua `border` + tag `MORPH — EVOLUÇÃO` (estilo SectionHeading). **Corpo em coluna única** (`.bp-legend-body`) apenas com os **dados** (`.bp-data`) — o diagrama (retângulo → seta → círculo) foi **removido** (feedback: "tire o diagrama da legenda, não faz sentido") — em 3 grupos empilhados rotulados **INICIAL** (W/H/d/A), **FINAL** (D/r/A=πr²/C) e **POSIÇÃO** (x·y/s), com `data-bp` preenchidos via JS.

### D7. Raio atual anotado sobre a imagem
O círculo-fantasma fica **centrado na imagem inicial, quase na borda inferior** (`left: 50%`, `bottom: 0.5rem`, feedback: "coloque o círculo ao centro da imagem inicial, mais pra baixo, quase na borda"). O **traçado diagonal tracejado com seta** que cruzava a imagem do canto do retângulo até o círculo foi **removido** (feedback: "tire o traçado que cruza a imagem até o centro do círculo, junto com a ponta da seta") — não há nenhuma linha cruzando a foto. **4 traços de cota nos pontos cardeais** do círculo (feedback: "tem 1 traço ao lado de r, colocar +3: um ao lado, um em cima, um abaixo"): o da esquerda (`.bp-ghost-r-line`, 1rem horizontal, na altura do centro) + **superior e inferior verticais** (`.bp-ghost-tick-top/bottom`, `rotate: 90deg`) e o da **direita horizontal** (`.bp-ghost-tick-right`, `right: -1.5rem`) — todos com 1rem e traço `--bp-line-strong`. O rótulo **`r 80px`** (`.bp-ghost-r-label`) fica **dentro do círculo, no quadrante inferior esquerdo** (`left: 25%`, `top: 59%`, feedback: "no meio do círculo, quadrante inferior esquerdo, não sendo cruzado por nenhuma linha"), **fora do cruzamento das linhas-guia** do crosshair, sem fundo sólido nem text-shadow (decisão do usuário: "sem text-shadow, mesma cor") — usa `var(--foreground)` pura sobre a foto.
